// Template: Segment Tree Beats
// Signal: one mutable array with online range chmin, range chmax, range add,
// and range sum queries, where plain lazy tags cannot summarize the clamp
// operations without richer node summaries.
// Assumes: zero-based half-open intervals [l, r); canonical beats lane only:
// range_chmin, range_chmax, range_add, and range_sum on long long values.
// Exposes: SegmentTreeBeats with build(a), range_chmin(), range_chmax(),
// range_add(), and range_sum().
// Complexity: O(n) build, O(log n) range_sum, O(log n) range_add, and
// amortized heavy near-logarithmic range_chmin/range_chmax updates.
// Main trap: calling the O(1) cap update when the strict second-extremum
// condition does not hold, or assuming this starter covers modulo/historic
// variants.
// Links:
//   Topic: topics/data-structures/segment-tree-beats/README.md
//   Note: practice/ladders/data-structures/segment-tree-beats/rangechminchmaxaddrangesum.md
...
int main() {
    ...
    st.range_chmin(0, 4, 4);  // [4, 1, 4, 3]
    st.range_add(1, 3, 2);    // [4, 3, 6, 3]
    st.range_chmax(0, 2, 5);  // [5, 5, 6, 3]

    cout << st.range_sum(0, 4) << '\n';
