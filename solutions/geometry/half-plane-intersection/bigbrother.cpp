...
long double signed_area2(const vector<Point> &poly) {
    long double s = 0;
    for (int i = 0; i < static_cast<int>(poly.size()); ++i) {
        s += cross(poly[i], poly[(i + 1) % static_cast<int>(poly.size())]);
    }
    return s;
}

long double polygon_area(const vector<Point> &poly) {
    return fabsl(signed_area2(poly)) / 2.0L;
}
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (ccw) {
            ...
            hps.emplace_back(b, a);
        }
    }

    vector<Point> kernel = halfplane_intersection(hps);
    long double area = polygon_area(kernel);
    if (area < 1e-15L) area = 0.0L;
