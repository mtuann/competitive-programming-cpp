...
using ll = long long;
const ll NEG_INF64 = -(1LL << 60);

/*
Problem: VMMARBLE - Phan loai bi
Judge: VN SPOJ
URL: https://vn.spoj.com/problems/VMMARBLE/
Topic: bitmask DP, exact residual-state DP

This file is a direct "make it exact-er" rewrite from the strong 95-point idea:
- keep the outer idea: choose the final keeper color of each box
- keep the K = 1 assignment DP
- replace the old pairwise heuristic evaluator by an exact residual evaluator

Residual model for K = 2, M <= 3:
- Bx: a box whose keeper color is x and both wrong colors are odd
- Sxy: a box whose keeper color is x and exactly one odd marble of color y remains

Packed 9 counters (6 bits each):
- idx 0..2: B0, B1, B2
- idx 3..8: S01, S02, S10, S12, S20, S21

Exact max_save is memoized on an 8-invariant compression of those 9 counters.

Important note:
- this is an exact direction for the official subtasks K = 1 and K = 2, M <= 3
- it is much closer to an exact solution than the old pairwise beam-search code
- but this file is still presented as a research/upgrade path, not a proven full-score claim
*/
...
struct SearchKey {
    ...
    bool operator==(const SearchKey& other) const {
        return delta9 == other.delta9 && idx == other.idx && mask == other.mask;
    }
};

struct SearchKeyHash {
    size_t operator()(const SearchKey& s) const noexcept {
        return static_cast<size_t>(s.delta9 * 239017ULL + s.idx * 131ULL + s.mask);
    }
};

struct Option {
    uint64_t delta9 = 0;
    unsigned char cover_mask = 0;
    int gain = 0;
    int local_upper = 0;
};
...
static Option make_option(const array<int, 3>& box, int home) {
    Option option;
    vector<int> odd_wrong;
    for (int color = 0; color < 3; ++color) {
        if (color != home && (box[color] & 1)) {
            odd_wrong.push_back(color);
        }
    }

    if (static_cast<int>(odd_wrong.size()) == 2) {
        option.delta9 += 1ULL << (6 * home);
    } else if (static_cast<int>(odd_wrong.size()) == 1) {
        const int color = odd_wrong[0];
        int id = -1;
    ...
    option.cover_mask = static_cast<unsigned char>(1 << home);
    option.gain = (box[home] + 1) / 2;
    option.local_upper = max_save8(pack8_from_9(option.delta9));
    return option;
}

static long long solve_k2_m3_exacter(const vector<array<int, 3>>& boxes) {
    int required_mask = 0;
    ...
    for (int pattern = 0; pattern < 8; ++pattern) {
            ...
            for (const auto& entry : current) {
                for (int home = 0; home < 3; ++home) {
                    ...
                    for (int mask = 0; mask < 8; ++mask) {
                        ...
                        const int next_mask = mask | options[home].cover_mask;
        ...
        for (const auto& entry : current) {
            for (int mask = 0; mask < 8; ++mask) {
                ...
                option.cover_mask = static_cast<unsigned char>(mask);
        ...
        sort(
            ...
            [](const Option& a, const Option& b) {
                if (a.gain + a.local_upper != b.gain + b.local_upper) {
                    return a.gain + a.local_upper > b.gain + b.local_upper;
                }
    ...
    sort(
        ...
        order.end(),
        [&](int lhs, int rhs) {
            const int score_l = groups[lhs][0].gain + groups[lhs][0].local_upper + 2 * group_box_counts[lhs];
            const int score_r = groups[rhs][0].gain + groups[rhs][0].local_upper + 2 * group_box_counts[rhs];
    ...
    vector<int> suffix_best_gain(group_count + 1, 0);
    for (int idx = group_count - 1; idx >= 0; --idx) {
        remaining_boxes[idx] = remaining_boxes[idx + 1] + group_box_counts[idx];
        suffix_best_gain[idx] = suffix_best_gain[idx + 1] + groups[idx][0].gain;
    }
    ...
    unordered_map<SearchKey, int, SearchKeyHash> seen;
    seen.reserve(1 << 20);
    ...
    function<void(int, uint64_t, int, int)> dfs = [&](int idx, uint64_t delta9, int mask, int gain) {
        ...
        if (gain + current_save + suffix_best_gain[idx] + remaining_boxes[idx] <= best_reward) {
            return;
        }

        const SearchKey search_key{delta9, static_cast<unsigned char>(idx), static_cast<unsigned char>(mask)};
        auto iter = seen.find(search_key);
        if (iter != seen.end() && iter->second >= gain) {
            return;
        }
        seen[search_key] = gain;
        ...
        for (const auto& option : groups[idx]) {
            dfs(idx + 1, delta9 + option.delta9, mask | option.cover_mask, gain + option.gain);
        }
    };
...
int main() {
    ...
    if (K == 1) {
        vector<ll> dp(1 << M, NEG_INF64), next(1 << M, NEG_INF64);
        dp[0] = 0;

        for (int i = 0; i < N; ++i) {
            fill(next.begin(), next.end(), NEG_INF64);
            for (int mask = 0; mask < (1 << M); ++mask) {
                if (dp[mask] <= NEG_INF64 / 2) {
                    continue;
                }
                for (int color = 0; color < M; ++color) {
                    const int next_mask = mask | (1 << color);
                    next[next_mask] = max(next[next_mask], dp[mask] + A[i][color]);
                }
            }
            dp.swap(next);
        }

        cout << (total - dp[need_mask]) << '\n';
        return 0;
    }
    ...
    for (int i = 0; i < N; ++i) {
        boxes[i] = {0, 0, 0};
        for (int color = 0; color < M; ++color) {
            boxes[i][color] = A[i][color];
        }
    }

    cout << solve_k2_m3_exacter(boxes) << '\n';
    return 0;
}
