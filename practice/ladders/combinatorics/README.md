# Combinatorics Ladders

These ladders focus on counting under structure and constraints.

## Subtopic Ladders

- [Counting basics](counting-basics/README.md)
- [Inclusion-exclusion](inclusion-exclusion/README.md)
- [Bounded compositions](bounded-compositions/README.md)
- [Lexicographic enumeration](lexicographic-enumeration/README.md)

## Representative Solved Notes

- [Distributing Apples](counting-basics/distributingapples.md)
- [Prime Multiples](inclusion-exclusion/primemultiples.md)
- [VOSFENCE](bounded-compositions/vosfence.md)
- [VOITSORT](lexicographic-enumeration/voitsort.md)
